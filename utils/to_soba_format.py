...
def handle_one_data(data_name, dataset_path, dtype):
    """
    Convert our dataset to soba format. Write to "annotations", "association_anno", etc.
    :param data_name: data's id number
    :param dataset_path: the path to dataset
    :param dtype: the type of dataset. including "indoor", "outdoor", "cross".
    :return: None
    """
    ...
    for idx in range(0, object_count):
        ...
        rle_obj = mask_util.encode(np.asarray(mask_arr, order="F"))
        RLEs.append(rle_obj.copy())
        rle_obj["counts"] = rle_obj["counts"].decode("utf-8")

        annotation_data["area"] = int(mask_util.area(rle_obj))
        annotation_data["segmentation"] = rle_obj
        annotation_data["bbox"] = mask_util.toBbox(rle_obj).tolist()
    ...
    for idx in range(0, object_count):
        ...
        rle_obj = mask_util.merge([RLEs[idx], RLEs[idx+object_count]])
        rle_obj["counts"] = rle_obj["counts"].decode("utf-8")

        association_anno_data["area"] = int(mask_util.area(rle_obj))
        association_anno_data["segmentation"] = rle_obj
    ...
    for dtype in dataset_types:
        ...
        for file in os.listdir(dt_path):
            ...
            if os.path.isfile(file_path):
                ...
                handle_one_data(file, dt_path, dtype)
