...
def integrateResult(file_path):
    ...
    # 获取第一张图片
    first_index_obj = Image.open(os.path.join(file_path, index_obj_file_names[0]))
    # 获取图片的宽度和高度
    ...
    for index_obj_file_name in index_obj_file_names:
        ...
        # 打开IndexObj图片
        index_obj = Image.open(os.path.join(file_path, index_obj_file_name))
    ...
    # 获取第一张图片
    first_shadow = Image.open(os.path.join(file_path, shadow_file_names[0]))
    # 获取图片的宽度和高度
    ...
    for shadow_file_name in shadow_file_names:
        ...
        curColor = colors.pop(0)
        # 打开shadow图片
        shadow = Image.open(os.path.join(file_path, shadow_file_name))
    ...
    # 将shadow和result合并
    result = Image.open(os.path.join(file_path, 'result.png'))
    shadow = Image.open(os.path.join(file_path, 'shadow_mask.png'))
    result.paste(shadow, (0, 0), shadow)
    result.save(os.path.join(file_path, 'object_shadow_mask.png'))

integrateResult(r'D:\Programming\python\SOSD\scripts\1')
